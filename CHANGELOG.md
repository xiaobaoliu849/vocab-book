...
### UI 界面修复与优化
- **学习热力图 (Heatmap) 高分屏适配**:
  - 修复在高 DPI 屏幕 (如 4K 显示器) 上热力图显示过小的问题
  - 动态计算 Canvas 高度，防止底部被截断
  - 根据缩放比例动态调整左侧和顶部边距，修复 "Mon/Wed/Fri" 标签显示不全的问题
  - 优化字体大小缩放逻辑，使文字与方块比例更协调
- **复习界面 (Review View) 布局修复**:
  - 修复右上角 "结束 (End)" 按钮在某些分辨率下边框显示不全的问题
  - 重构顶部 Header 布局，使用独立容器管理右侧按钮组
  - 调整按钮间距和宽度，防止挤压截断
